...
const Footer = () => {
  ...
  const sendMessage = () => {
    ...
    const userId = userInfo ? userInfo.payload.userId : 0;
    const message = document.getElementById("txt-chat").value;
      ...
      <div className="chat-popup" id="formChat">
        ...
        <ol className="discussion" id="chat-noiDung">
          {dataChat.map((item) => {
            ...
            const userId = userInfo ? userInfo.payload.userId : 0;
            return (
              <li className={item.userId === userId ? "self" : "other"}>
